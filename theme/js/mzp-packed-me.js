...
var MagicZoom_ua='msie';var W=navigator.userAgent.toLowerCase();if(W.indexOf("opera")!=-1){
    ...
    if(this.loadingCont!=null&&(!this.bigImage.complete||0==this.bigImage.width||0==this.bigImage.height)&&this.smallImage.width!=0&&this.smallImage.height!=0){
        ...
        this.bigImageCont.style.left=this.smallImageSizeX+parseInt(MagicZoom_getStyle(this.smallImage,'borderLeftWidth'))+parseInt(MagicZoom_getStyle(this.smallImage,'borderRightWidth'))+parseInt(MagicZoom_getStyle(this.smallImage,'paddingLeft'))+parseInt(MagicZoom_getStyle(this.smallImage,'paddingRight'))+5+'px'
        }else{
        this.bigImageCont.style.left=(r['right']-r['left']+5)+'px'
        }switch(this.settings['position']){
        case'left':this.bigImageCont.style.left='-'+(5+parseInt(this.bigImageCont.style.width))+'px';break;case'bottom':if(r){
            this.bigImageContStyleTop=r['bottom']-r['top']+5+'px'
            }else{
            this.bigImageContStyleTop=this.smallImage.height+5+'px'
            }this.bigImageCont.style.left='0px';break;case'top':this.bigImageContStyleTop='-'+(5+parseInt(this.bigImageCont.style.height))+'px';this.bigImageCont.style.left='0px';break;case'custom':this.bigImageCont.style.left='0px';this.bigImageContStyleTop='0px';break;case'inner':this.bigImageCont.style.left='0px';this.bigImageContStyleTop='0px';if(this.settings['zoomWidth']==-1){
    ...
    var loadingText='Loading Zoom';var loadingImg='';var iels=window.document.getElementsByTagName("IMG");for(var i=0;i<iels.length;i++){
        ...
        if(/MagicZoom/.test(aels[i].className)){
            while(aels[i].firstChild){
                ...
                }re=new RegExp(/zoom\-height(\s+)?:(\s+)?(\w+)/i);var zoomHeight=-1;matches=re.exec(aels[i].rel);bigCont.style.height='301px';if(matches){
    ...
    adjustPosition:function(ps){
        var padW=parseInt(MagicTools.Element.getStyle(this.cont,'padding-left'))+parseInt(MagicTools.Element.getStyle(this.cont,'padding-right'))+parseInt(MagicTools.Element.getStyle(this.cont,'border-left-width'))+parseInt(MagicTools.Element.getStyle(this.cont,'border-right-width')),padH=parseInt(MagicTools.Element.getStyle(this.cont,'padding-top'))+parseInt(MagicTools.Element.getStyle(this.cont,'padding-bottom'))+parseInt(MagicTools.Element.getStyle(this.cont,'border-top-width'))+parseInt(MagicTools.Element.getStyle(this.cont,'border-bottom-width'));var destTop=destLeft=0;MagicTools.Element.setStyle(this.bigImg,{
            ...
            var sRect=MagicTools.Element.getRect(this.smallImg);destTop=sRect.bottom-Math.round((sRect.bottom-sRect.top)/2)-Math.round(imgSize.height/2);if(destTop+imgSize.height+this.caption.fullHeight>ps.height+ps.scrollY-5){
                destTop=ps.height+ps.scrollY-5-imgSize.height-this.caption.fullHeight
                }if(destTop<ps.scrollY+10){
                destTop=ps.scrollY+10
                }destLeft=Math.round(sRect.right-(sRect.right-sRect.left)/2-imgSize.width/2);if(destLeft+imgSize.width>ps.width+ps.scrollX-5){
                destLeft=ps.width+ps.scrollX-imgSize.width-5
                }if(destLeft<ps.scrollX+10){
                destLeft=ps.scrollX+10
                }
            ...
            var sRect=MagicTools.Element.getRect(this.smallImg);if('auto'==this.options.zoomPositionOffset.top){
                ...
                }if(destTop+imgSize.height+this.caption.fullHeight>ps.height+ps.scrollY-5){
                destTop=ps.height+ps.scrollY-5-imgSize.height-this.caption.fullHeight
                }if(destTop<ps.scrollY+10){
                destTop=ps.scrollY+10
                }if(destLeft+imgSize.width>ps.width+ps.scrollX-5){
                destLeft=ps.width+ps.scrollX-imgSize.width-5
                }if(destLeft<ps.scrollX+10){
                destLeft=ps.scrollX+10
                }
